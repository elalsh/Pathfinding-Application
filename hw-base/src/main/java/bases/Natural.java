...
public class Natural {
    ...
    public static int leadingDigit(int[] digits) {
        ...
        // Inv : digits[i+1] = 0, ,.., digits[n-1] = 0
        while (i != 0) {
            if (digits[i] != 0) {
                // At this point in the code, we know that digits[i+1],,,digits[n-1] are all zero
                // and D[i] != 0.
                // This implies the postcondition below, since digits[i+1],,,digits[n-1] are all zero, it
                // satisfies the first assertion of the post condition
                //  and D[i] != 0 satisfies the second assertion since D[i] != 0 is true and only one of
                //  D[i] != 0 or i = 0 has to be true.
                return i;
            }
            i = i - 1;
        }
        ...
        // This implies the postcondition below, since digits[i+1],,,digits[n-1] are all zero, it
        // satisfies the first assertion of the post condition
        // and i = 0 satisfies the second assertion since i = 0 is true and D[i] != 0 or D[i] = 0
        // but only one of D[i] != 0 or i = 0 has to be true.
    ...
    public Natural toBase(int base) {
        ...
        // At this point we know that i + j = n-1, j = this.digits.length = n-1, i = 0 and
        // Inv: r = (base, D[i] b^0 + D[i+1] b^1 + ... + D[n-1] b^j) and i+j = n-1,
        //      where D = this.digits, n = this.digits.length, and b = this.base.
        // This implies the postconditin since
        // D[i] b^0 + D[i+1] b^1 + .. + D[n-1] b^j = D[0] b^0 + D[i+1] b^1 + ..+ D[n-1] b^{n-1} = this.value)
        // thus, r = (base, this.value)
    ...
    public String toString() {
        ...
        int i = 0;
        // Inv : buf = ch(D[i-1]), ch(D[i-2]), .., ch(D[0])
        while (i != this.digits.length) {
            buf.insert(0,BaseDigits.digitToChar(this.digits[i], this.base));
            i = i + 1;
        }

        // At this point in the code, we know that i = this.digits.length = n.
        // This implies the postcondition below, since buf contains all the digits corresponding to
        // this value in this base in the order of right to left of this.digits,
        // as in ch(D[i-1]), ch(D[i-2]), .., ch(D[0]) when read from left to right.
    ...
    public Natural plus(Natural other) {
        ...
        // It is setting the value of D to the sum of the value of A and the value of B for all indexes
        // from 0 to i-1
        ...
        // 0 <= A[k] < b and 0 <= B[k] < b for any index k, 0 <= k < other.digits.length, the maximum sum possible at each index
        // is 2b-2 since the max for A[k] = b-1 and B[k] = b-1, which is always < 2b-1.
        // If this.length is longer than other.length, all index greater than or equal to
        // other.digits.length will be 0 by default

        // The invariant holds initially because the first loop sets the value of D
        // to D[0] = A[0]+B[0], D[1] = A[1]+B[1], ..., D[i-1] = A[i-1]+B[i-1], where k < other.digits.length.
        //

        // It is setting the value of D[i] = A[i] + B[i] for indexes from i to n-1.
        // If i >= other.digits.length, we are setting D[i] = A[i] + 0, since we don't have a
        // a value at B[i]. This for the case of this.digits.length being longer than the other.digits.length.
        //
        // Inv: D[0] = A[0]+B[0], D[1] = A[1]+B[1], ..., D[i-1] = A[i-1]+B[i-1],
        ...
        // this.value + other.value = D[0] + D[1] b + ... + D[n-1] b^{n-1} because this.value returns
        // A[0] + A[1]b + A[n-1]^{n-1} and other.value also returns B[0] + B[1]b + .. +B[k]-1b^{k-1}, adding them
        // together, for each index i we get A[i]b^i + B[i]b^i = (A[i] + B[i])b^i = D[i]b^i,
        // where B[k] = other.digits[k] if i < other.digits.length
        //       B[k] = 0 otherwise
        //
        ...
        //        //      D[0] < b, D[1] < b, ..., D[1] < b and
        //        //      D[2] < b and D[n-2] < b-1, D[n-1] < b-1.
        //  (1) This implies the postcondition below since newDigits = this.value + other.value
        //    and uses the same base.
        // (2) the preconditions of this constructor holds since 2 <= this.base <= 36 as required by the preconditon
        // of the method. digits.length >= 1 since we initializes it's length as [this.digits.length + 1] and
        //      all digits of newDigits are between 0 and base - 1 by what we know after exiting the loop.
